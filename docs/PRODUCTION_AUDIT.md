...
| **Async Database Interface Mismatch** | Critical | Unified `MemoryStore` and `PostgresDatabase` under strictly typed `Promise` contracts; audited and awaited every call site across all routes and middleware | `tests/security/database-real-postgres.test.ts` (Rule 9 test), 98/98 tests pass | **RESOLVED** |
...
| **Backup / Restore Verification** | High | Added automated logical backup, clean-target restore, and state integrity drill | `tests/integration/backup-restore.test.ts`, `scripts/test-backup-restore.cjs` | **RESOLVED** |
