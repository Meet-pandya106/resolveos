...
describe('Security & Offline Sync: Batch Ingestion, Idempotency & 3-Way Differential Merge', () => {
  ...
  beforeAll(async () => {
    ...
    const reg = await server.inject({
      ...
      payload: { email: 'sync.tester@resolveos.test', password: 'Password#Sync2026!', name: 'Sync Tester' }
    });
        ...
        mutations: [
          {
            ...
            entityId: caseId,
            action: 'UPDATE',
            baseSnapshot: { title: 'Distributed Database Partition', description: 'Node cluster desynchronized' },
            localSnapshot: { title: 'Distributed Database Partition - Triage Complete', description: 'Node cluster desynchronized' },
