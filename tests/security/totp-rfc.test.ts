...
describe("Security: RFC 6238 TOTP Authenticator Standards Compliance", () => {
	...
	it("matches official RFC 6238 test vectors across multiple epochs", () => {
		...
		expect(TOTPService.generateCode(rfcSecret, 1111111109 * 1000)).toBe(
			"081804",
		);

		// Epoch: 1111111111s -> 050471
		expect(TOTPService.generateCode(rfcSecret, 1111111111 * 1000)).toBe(
			"050471",
		);

		// Epoch: 1234567890s -> 005924
		expect(TOTPService.generateCode(rfcSecret, 1234567890 * 1000)).toBe(
			"005924",
		);

		// Epoch: 2000000000s -> 279037
		expect(TOTPService.generateCode(rfcSecret, 2000000000 * 1000)).toBe(
			"279037",
		);
	});
