...
import { initDatabase, auditEvents, getDatabase } from '../../packages/database/src/index.js';
...
describe('Tamper-Evident Cryptographic Audit Log', () => {
  ...
  it('computes valid SHA-256 hash chains for successive security events', () => {
    const h1 = AuditService.log('LOGIN', { userId: 'u-1', details: { ip: '1.2.3.4' } });
    const h2 = AuditService.log('ROLE_CHANGED', { userId: 'u-1', workspaceId: 'w-1', details: { role: 'ADMIN' } });
    const h3 = AuditService.log('CASE_EXPORTED', { userId: 'u-1', workspaceId: 'w-1', details: { format: 'JSON' } });
  ...
  it('detects tampering when an audit event payload is modified in place', () => {
    ...
    AuditService.log('CASE_EXPORTED', { userId: 'u-1', details: { format: 'JSON' } });
  ...
  it('detects tampering when a block is spliced or previousHash is forged', () => {
    ...
    AuditService.log('CASE_EXPORTED', { userId: 'u-1', details: { title: 'Case A' } });

    const db = getDatabase();
    const allEvents = db.select().from(auditEvents).all();

    // Maliciously modify the previousHash pointer
    (allEvents[1] as any).previousHash = 'forged_hash_00000000000000000000000000000000000000000000000000000000';
