...
describe("Security: SSRF & URL Security Guard", () => {
	...
	it("blocks cloud instance metadata endpoints", () => {
		...
		expect(
			SSRFGuard.isSafeUrl("http://metadata.google.internal/computeMetadata/v1/")
				.safe,
		).toBe(false);
		expect(
			SSRFGuard.isSafeUrl("http://instance-data/latest/meta-data/").safe,
		).toBe(false);
	});
	...
	it("blocks private and link-local IPv6 ranges", () => {
		...
		expect(SSRFGuard.isSafeUrl("http://[fd12:3456:789a:1::1]/").safe).toBe(
			false,
		);
	});
