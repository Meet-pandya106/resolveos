...
describe("Security: Comprehensive Cross-Tenant IDOR & BOLA Regression Suite", () => {
	...
	beforeAll(async () => {
		...
		const caseRes = await server.inject({
			...
			payload: {
				title: "Secret Vulnerability Report",
				severity: "CRITICAL",
				description: "Zero-day exploit notes",
			},
		});
		...
		const hypRes = await server.inject({
			...
			headers: { authorization: `Bearer ${userAToken}` },
			payload: {
				description: "Compromised service account token",
				confidence: "HIGH",
				status: "UNTESTED",
			},
		});
		...
		const actRes = await server.inject({
			...
			headers: { authorization: `Bearer ${userAToken}` },
			payload: {
				title: "Rotate root cryptographic secrets",
				priority: "URGENT",
			},
		});
