...
describe('Security: Password Hashing & Verification', () => {
  ...
  it('verifies correct passwords and rejects incorrect passwords', async () => {
    ...
    const isValid = await SecurityCrypto.verifyPassword('ValidPassword#2026', hash);
    const isInvalid = await SecurityCrypto.verifyPassword('WrongPassword#2026', hash);
...
describe('Security: Sensitive Data & PII Redaction', () => {
  it('redacts emails, API keys, phone numbers, and IP addresses', () => {
    const rawText = 'Contact devon@company.com with key sk-abcdef1234567890123456 at 192.168.1.50 or call (555) 123-4567';
...
describe('Security: SSRF & URL Security Guard', () => {
  ...
  it('allows valid public HTTPS endpoints', () => {
    ...
    expect(SSRFGuard.isSafeUrl('https://www.example.com/status').safe).toBe(true);
  });
});
...
describe('Security: Path Sanitizer', () => {
  it('strips path traversal sequences from filenames', () => {
    ...
    expect(PathSanitizer.sanitizeFilename('..\\..\\windows\\system32\\cmd.exe')).toBe('windows_system32_cmd.exe');
  });
});
