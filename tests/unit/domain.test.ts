...
describe("Domain: Case State Machine", () => {
	it("allows valid state transitions", () => {
		...
		expect(CaseStateMachine.canTransition("INVESTIGATING", "VERIFYING")).toBe(
			true,
		);
	...
	it("enforces verification check before resolving unless override is granted", () => {
		const unverified = CaseStateMachine.validateTransition(
			"VERIFYING",
			"RESOLVED",
			{
		...
		const verified = CaseStateMachine.validateTransition(
			"VERIFYING",
			"RESOLVED",
			{
...
describe("Domain: Problem Statement Quality Scorer", () => {
	...
	it("calculates comprehensive score for complete statement", () => {
		const res = ProblemScorer.calculateScore({
			...
			statement:
				"During the flash sale event, latency exceeded 4.8 seconds for checkout transactions.",
			expectedBehavior: "Transactions acknowledge under 250ms at p99.",
			observedBehavior:
				"Transactions timed out after 5,000ms with connection pool exhaustion.",
...
describe("Domain: Conflict Resolution", () => {
	it("merges non-conflicting field mutations cleanly", () => {
		...
		const remote = {
			title: "Base Title",
			description: "Updated Remote Desc",
			severity: "MEDIUM",
		};
