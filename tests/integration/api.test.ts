...
describe('Integration: Complete Problem Resolution Lifecycle', () => {
    ...
    const res = await server.inject({
      ...
      payload: {
        problemStatement: {
          ...
          statement: 'The checkout gateway experienced severe response degradation during traffic surge.',
          expectedBehavior: 'Transactions acknowledge under 250ms at p99.',
          observedBehavior: 'Latency surged to 4.85 seconds with connection exhaustion.',
  ...
  it('7. Logs an immutable decision with chosen solution', async () => {
    const res = await server.inject({
      ...
      payload: {
        ...
        reasoning: 'PgBouncer immediately decouples pod concurrency spikes from backend DB limits.',
  ...
  it('9. Registers a verification criterion and marks it PASSED', async () => {
    const res = await server.inject({
      ...
      headers: { authorization: `Bearer ${token}` },
      payload: {
        expectedResult: 'p99 latency < 200ms during synthetic load test of 5k RPS',
  ...
  it('11. Generates and publishes retrospective', async () => {
    const res = await server.inject({
      ...
      headers: { authorization: `Bearer ${token}` },
      payload: {
        whatHappened: 'Payment API gateway suffered latency degradation during flash sale.',
