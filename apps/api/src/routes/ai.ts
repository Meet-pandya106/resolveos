...
export const aiRoutes: FastifyPluginAsync = async (server: FastifyInstance) => {
  ...
  server.post('/:workspaceId/cases/:caseId/analyze', { preHandler: [requireWorkspaceAccess()] }, async (request, reply) => {
    ...
    const consent = db
      .select()
      .from(consentRecords)
      .where(and(eq(consentRecords.userId, request.user!.id), eq(consentRecords.consentType, 'AI_PROCESSING')))
      .get();
    ...
    if (!consent || consent.status !== 'GRANTED') {
      return reply.status(403).send({
        statusCode: 403,
        error: 'Forbidden',
        message: 'Explicit user consent for AI processing is required. Please enable in Privacy Center.',
        requestId: request.id
      });
    }
