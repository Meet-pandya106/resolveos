...
export const caseRoutes: FastifyPluginAsync = async (
	...
	server.post(
		"/:workspaceId/cases",
		{ preHandler: [requireWorkspaceAccess(["OWNER", "ADMIN", "MEMBER"])] },
		async (request, reply) => {
			const { workspaceId } = request.params as { workspaceId: string };
			...
			if (problemStmt) {
				...
				problemStmt = {
					...problemStmt,
					completenessScore: scoreRes.score,
				} as any;
			}
			...
			RealtimeService.broadcastToWorkspace(workspaceId, "CASE_CREATED", {
				caseId,
				title: body.title,
			});
			...
			return reply
				.status(201)
				.send({ id: caseId, message: "Case created successfully" });
		},
	);

	// 3. Get Single Case Details with Aggregate Overview
	server.get(
		"/:workspaceId/cases/:caseId",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			...
			const problemScore = ProblemScorer.calculateScore(
				caseItem.problemStatement as any,
			);
		...
		async (request, reply) => {
			...
			if (body.expectedVersion && body.expectedVersion !== existing.version) {
				return reply.status(409).send({
					...
					error: "Conflict",
					message:
						"Case has been modified by another user. Please refresh and review latest changes.",
					requestId: request.id,
				});
			}
			...
			if (body.status && body.status !== existing.status) {
				const verificationsList = await db
					...
					.all();
				const hasPassed = verificationsList.some(
					(v: any) => v.status === "PASSED",
				);

				const isAuthorizedRole =
					request.workspaceMember?.role === "OWNER" ||
					request.workspaceMember?.role === "ADMIN";
				const hasExplicitOverride = Boolean(
					body.overrideVerification && body.overrideReason && isAuthorizedRole,
				);

				const transitionCheck = CaseStateMachine.validateTransition(
			...
			await db.transaction(async (tx) => {
				await tx.update(cases)
					.set({
						...
						incidentMode:
							body.incidentMode !== undefined
								? body.incidentMode
								: existing.incidentMode,
			...
			RealtimeService.broadcastToCase(caseId, "CASE_UPDATED", {
				caseId,
				updates: body,
			});
			return reply.send({
	...
	server.get(
		"/:workspaceId/cases/:caseId/evidence",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.post(
		...
		async (request, reply) => {
				...
				const existing = await db
					...
					.where(
						and(
							...
							eq(caseEvidence.title, body.title),
						),
					)
					.get();
				if (existing) {
					return reply
						...
						.send({
							id: existing.id,
							message: "Evidence already recorded (idempotent)",
							idempotent: true,
						});
				}
			}
			...
			RealtimeService.broadcastToCase(caseId, "EVIDENCE_ADDED", {
				id,
				title: body.title,
			});
			...
			return reply
				.status(201)
				.send({ id, message: "Evidence recorded successfully" });
		},
	);

	// 6. Evidence Relationships (IDOR Protected)
	server.get(
		"/:workspaceId/cases/:caseId/relationships",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.post(
		...
		async (request, reply) => {
			...
			return reply
				.status(201)
				.send({ id, message: "Relationship established" });
		},
	);

	// 7. Questions (IDOR Protected)
	server.get(
		"/:workspaceId/cases/:caseId/questions",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.patch(
		"/:workspaceId/cases/:caseId/questions/:questionId",
		{ preHandler: [requireWorkspaceAccess(["OWNER", "ADMIN", "MEMBER"])] },
		async (request, reply) => {
			const { workspaceId, caseId, questionId } = request.params as {
				workspaceId: string;
				caseId: string;
				questionId: string;
			};
			...
			await db.update(caseQuestions)
				...
				.where(
					and(
						eq(caseQuestions.id, questionId),
	...
	server.get(
		"/:workspaceId/cases/:caseId/hypotheses",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.patch(
		"/:workspaceId/cases/:caseId/hypotheses/:hypothesisId",
		{ preHandler: [requireWorkspaceAccess(["OWNER", "ADMIN", "MEMBER"])] },
		async (request, reply) => {
			const { workspaceId, caseId, hypothesisId } = request.params as {
				workspaceId: string;
				caseId: string;
				hypothesisId: string;
			};
			...
			await db.update(hypotheses)
				...
				.where(
					and(eq(hypotheses.id, hypothesisId), eq(hypotheses.caseId, caseId)),
				)
				.run();
	...
	server.get(
		"/:workspaceId/cases/:caseId/root-causes",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.get(
		"/:workspaceId/cases/:caseId/solutions",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.get(
		"/:workspaceId/cases/:caseId/decisions",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.post(
		...
		async (request, reply) => {
			...
			return reply
				.status(201)
				.send({ id, revision: nextRevision, message: "Decision logged" });
		},
	);

	// 12. Actions (IDOR Protected)
	server.get(
		"/:workspaceId/cases/:caseId/actions",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.patch(
		"/:workspaceId/cases/:caseId/actions/:actionId",
		{ preHandler: [requireWorkspaceAccess(["OWNER", "ADMIN", "MEMBER"])] },
		async (request, reply) => {
			const { workspaceId, caseId, actionId } = request.params as {
				workspaceId: string;
				caseId: string;
				actionId: string;
			};
			...
			await db.update(caseActions)
				...
				.where(
					and(eq(caseActions.id, actionId), eq(caseActions.caseId, caseId)),
				)
				.run();
	...
	server.get(
		"/:workspaceId/cases/:caseId/verifications",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.post(
		...
		async (request, reply) => {
			...
			return reply
				.status(201)
				.send({ id, message: "Verification record initialized" });
		},
	);

	// 14. Retrospectives (IDOR Protected)
	server.get(
		"/:workspaceId/cases/:caseId/retrospective",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
	...
	server.get(
		"/:workspaceId/cases/:caseId/activities",
		{ preHandler: [requireWorkspaceAccess()] },
		async (request, reply) => {
			const { workspaceId, caseId } = request.params as {
				workspaceId: string;
