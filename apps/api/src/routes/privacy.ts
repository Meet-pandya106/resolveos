...
export const privacyRoutes: FastifyPluginAsync = async (
	...
	server.post("/consent", async (request, reply) => {
		...
		const existing = await db
			...
			.where(
				and(
					...
					eq(consentRecords.consentType, body.consentType),
				),
			)
			.get();
		...
		return reply.send({
			message: `Consent for ${body.consentType} updated to ${body.status}`,
		});
	});
		...
		async (request, reply) => {
			try {
				...
				if (body.format === "CSV") {
					...
					reply.header(
						"Content-Disposition",
						`attachment; filename="resolveos-export-${workspaceId}.csv"`,
					);
	...
	server.delete("/account", async (request, reply) => {
		...
		if (!validPassword) {
			AuditService.log("FAILED_LOGIN", {
				req: request,
				userId: user.id,
				details: { reason: "Incorrect password for deletion" },
			});
			return reply
				.status(403)
				.send({ error: "Invalid password. Account deletion aborted." });
		}
		...
		return reply.send({
			message:
				"Account successfully scheduled for permanent deletion and personal data anonymized.",
		});
	});
};
