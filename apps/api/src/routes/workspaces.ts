...
export const workspaceRoutes: FastifyPluginAsync = async (server: FastifyInstance) => {
  ...
  server.get('/', async (request, reply) => {
    ...
    const list = db
      ...
      .where(and(eq(workspaceMembers.userId, request.user!.id), eq(workspaces.deletedAt, null as any)))
      .all();

    return reply.send(list.map((item: any) => ({ ...item.workspace, role: item.role })));
  });
  ...
  server.get('/:workspaceId', { preHandler: [requireWorkspaceAccess()] }, async (request, reply) => {
  ...
  server.patch('/:workspaceId', { preHandler: [requireWorkspaceAccess(['OWNER', 'ADMIN'])] }, async (request, reply) => {
    const { workspaceId } = request.params as { workspaceId: string };
  ...
  server.get('/:workspaceId/members', { preHandler: [requireWorkspaceAccess()] }, async (request, reply) => {
    const { workspaceId } = request.params as { workspaceId: string };
    const db = getDatabase();
  ...
  server.post('/:workspaceId/members', { preHandler: [requireWorkspaceAccess(['OWNER', 'ADMIN'])] }, async (request, reply) => {
    const { workspaceId } = request.params as { workspaceId: string };
    const body = AddMemberInputSchema.parse(request.body);
    const db = getDatabase();

    const targetUser = db.select().from(users).where(eq(users.email, body.email.toLowerCase())).get();
    if (!targetUser) {
    ...
    return reply.status(201).send({ message: 'Member added successfully', memberId });
  });

  // 7. List Workspace API Keys
  server.get('/:workspaceId/api-keys', { preHandler: [requireWorkspaceAccess(['OWNER', 'ADMIN'])] }, async (request, reply) => {
    const { workspaceId } = request.params as { workspaceId: string };
    const db = getDatabase();
    const keys = db.select().from(apiKeys).where(and(eq(apiKeys.workspaceId, workspaceId), eq(apiKeys.isRevoked, false))).all();
  ...
  server.post('/:workspaceId/api-keys', { preHandler: [requireWorkspaceAccess(['OWNER', 'ADMIN'])] }, async (request, reply) => {
    const { workspaceId } = request.params as { workspaceId: string };
    ...
    return reply.status(201).send({
      ...
      keyFingerprint,
      secretKey: rawSecret,
      notice: 'Store this secret key safely. It will never be displayed again.'
    });
  });
};
