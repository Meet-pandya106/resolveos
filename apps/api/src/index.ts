...
const jwtSecret = process.env.JWT_SECRET || 'resolveos_default_development_jwt_secret_key_32_chars';
const sessionSecret = process.env.SESSION_SECRET || 'resolveos_default_development_session_secret_key_32_chars';
...
server.register(async function (fastify) {
  fastify.get('/ws', { websocket: true }, async (connection, req) => {
    try {
      ...
      if (!token) {
        connection.close(4401, 'Authentication token required for WebSocket connection');
        return;
      }

      const decoded = (await (server as any).jwt.verify(token)) as { id: string; email: string };
      const workspaceId = url.searchParams.get('workspaceId') || undefined;
      RealtimeService.registerClient(connection, decoded.id, workspaceId);
    } catch (err) {
      connection.close(4401, 'Invalid authentication token');
    }
  });
});
