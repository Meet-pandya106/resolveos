...
import { getDatabase, users, workspaces, workspaceMembers, userSessions, eq, and } from '@resolveos/database';
...
export async function authenticate(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    // Verify JWT payload
    const decoded = (await (request as any).jwtVerify()) as { id: string; email: string; sessionId?: string };
    const db = getDatabase();

    // Check user exists and is not soft-deleted
    const user = db.select().from(users).where(and(eq(users.id, decoded.id), eq(users.deletedAt, null as any))).get();
    if (!user) {
      return reply.status(401).send({
        statusCode: 401,
        error: 'Unauthorized',
    ...
    if (decoded.sessionId) {
      const session = db
        .select()
        .from(userSessions)
        .where(and(eq(userSessions.id, decoded.sessionId), eq(userSessions.isRevoked, false)))
        .get();
      ...
      if (session.expiresAt && new Date(session.expiresAt) <= new Date()) {
        db.update(userSessions).set({ isRevoked: true }).where(eq(userSessions.id, decoded.sessionId)).run();
        return reply.status(401).send({
          statusCode: 401,
          error: 'Unauthorized',
 ...
 */
export function requireWorkspaceAccess(allowedRoles: UserRole[] = ['OWNER', 'ADMIN', 'MEMBER', 'VIEWER']) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user as AuthenticatedUser | undefined;
    if (!user) {
      return reply.status(401).send({ statusCode: 401, error: 'Unauthorized', message: 'Authentication required', requestId: request.id });
    }
    ...
    if (!workspaceId) {
      return reply.status(400).send({ statusCode: 400, error: 'Bad Request', message: 'Workspace ID required', requestId: request.id });
    }
