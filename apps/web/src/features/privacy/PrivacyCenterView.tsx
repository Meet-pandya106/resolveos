...
export const PrivacyCenterView: React.FC = () => {
	...
	const handleToggleConsent = async (
		type: "AI_PROCESSING" | "ERROR_LOGGING",
		nextStatus: boolean,
	) => {
		try {
			...
			addToast({
				type: "success",
				message: `Privacy consent for ${type} updated.`,
			});
		} catch (err: any) {
		...
		<div className="max-w-4xl mx-auto space-y-6">
			<div>
				...
				<p className="text-xs text-muted-foreground mt-1">
					Zero third-party trackers. Explicit consent management and complete
					data ownership.
				</p>
			</div>
			...
			<div className="p-6 rounded-xl bg-card border border-border space-y-4">
				...
				<div className="space-y-4 pt-2">
					<div className="flex items-center justify-between p-4 rounded-lg bg-muted/20 border border-border">
						<div className="space-y-1">
							<span className="text-sm font-semibold">
								Optional AI Assistant Processing
							</span>
							<p className="text-xs text-muted-foreground max-w-lg">
								Allows sending problem statements through PII redaction filters
								for automated hypothesis and root cause suggestions.
							</p>
						</div>
						<input
							type="checkbox"
							checked={aiConsent}
							onChange={(e) =>
								handleToggleConsent("AI_PROCESSING", e.target.checked)
							}
					...
					<div className="flex items-center justify-between p-4 rounded-lg bg-muted/20 border border-border">
						<div className="space-y-1">
							<span className="text-sm font-semibold">
								Local Error & Diagnostic Logging
							</span>
							<p className="text-xs text-muted-foreground max-w-lg">
								Stores client-side crash telemetry locally in your browser's
								IndexedDB store without sending data to third parties.
							</p>
						</div>
						<input
							type="checkbox"
							...
							onChange={(e) =>
								handleToggleConsent("ERROR_LOGGING", e.target.checked)
							}
							className="w-4 h-4 rounded text-primary cursor-pointer"
						/>
					</div>
			...
			<div className="p-6 rounded-xl bg-card border border-border space-y-4">
				...
				<p className="text-xs text-muted-foreground">
					Download all cases, evidence artifacts, decisions, and action items in
					structured, unencrypted formats.
				</p>
			...
			<div className="p-6 rounded-xl bg-card border border-rose-500/30 bg-rose-500/5 space-y-4">
				...
				<p className="text-xs text-muted-foreground">
					Permanently anonymizes your personal information and revokes all
