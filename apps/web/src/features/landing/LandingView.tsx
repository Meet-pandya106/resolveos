...
Client IP: 192.168.1.104 requested password reset for card 4532-8921-9902-1234.`,
	...
	const redactSandbox = (text: string) => {
		return text
			...
			.replace(
				/(?:sk_live_|sk_test_|ghp_|gho_|xox[baprs]-)[a-zA-Z0-9]{12,}/g,
				"[SECRET_REDACTED]",
			)
			.replace(
	...
	const handleQuickDemoLogin = async () => {
		...
		try {
			...
			addToast({
				type: "success",
				message: "Welcome to the ResolveOS Live Demo Environment!",
			});
			navigate("/");
		} catch (err: any) {
			addToast({
				type: "error",
				message: "Demo server is offline. Starting local fallback...",
			});
			navigate("/login");
		} finally {
	...
	const stages = [
		{
			...
			preview: {
				...
				detail:
					"Scope: High-throughput ingestion nodes • Trigger: Burst connection reconnection cycles.",
			},
		},
		{
			num: 2,
			...
			preview: {
				...
				detail:
					"Expected: p99 < 45ms at 25k req/s. Observed: p99 peaked at 885ms with 14.2% socket drops.",
			},
		},
		{
			num: 3,
			...
			preview: {
				...
				detail:
					"Heap Dump (.heapsnapshot) verified 98% confidence • Contradictory Network Trace flagged 22% confidence.",
			},
		},
		{
			num: 4,
			...
			preview: {
				...
				detail:
					"Assigned to @sarah: Was the v2.4 socket buffer patch deployed prior to the 14:00 UTC traffic spike?",
			},
		},
		{
			num: 5,
			...
			preview: {
				...
				detail:
					"Hypothesis A (V8 Buffer Retain Cycle) CONFIRMED via memory profiling. Hypothesis B (OS File Descriptor leak) DISPROVEN.",
			},
		},
		{
			num: 6,
			...
			preview: {
				...
				detail:
					"Why 1: Memory exceeded cgroup limit → Why 2: Socket listeners not detached on client reset → Root: Missing finally unbind.",
			},
		},
		{
			num: 7,
			...
			preview: {
				...
				detail:
					"Option 1: Complete Broker Rewrite (Score: 42) vs Option 2: WeakRef Buffer Pool (Score: 94 - Recommended).",
			},
		},
		{
			num: 8,
			...
			preview: {
				...
				detail:
					"Decision: Adopt WeakRef WeakMap pooling. Rejected Alternative: Manual ref-counting due to high regression risk.",
			},
		},
		{
			num: 9,
			...
			preview: {
				...
				detail:
					"Task 1: Patch socket unbind in staging (DONE) • Task 2: Canary rollout to cluster us-east-1 (DONE).",
			},
		},
		{
			num: 10,
			...
			preview: {
				...
				detail:
					"Observed p99 after patch: 38ms (Target: < 45ms). Heap allocation stable at 410MB under 50k simulated socket connections.",
			},
		},
		{
			num: 11,
			...
			preview: {
				...
				detail:
					"State transition validated: Invariant check confirmed 100% test pass. Tamper-evident audit log stamped.",
			},
		},
		{
			num: 12,
			...
			preview: {
				...
				detail:
					"Timeline synthesized across 14 events. Preventive synthetic monitoring alert configured at p99 > 50ms.",
			},
		},
	];
		...
		<div className="min-h-screen bg-background text-foreground cyber-grid relative selection:bg-primary/30 selection:text-primary">
			...
			<header className="sticky top-0 z-50 backdrop-blur-xl bg-background/80 border-b border-border/80 px-6 py-3.5">
				<div className="max-w-7xl mx-auto flex items-center justify-between">
					...
					<nav className="hidden md:flex items-center gap-6 text-xs font-medium text-muted-foreground">
						...
						<a href="#sandbox" className="hover:text-primary transition-colors">
							PII Redactor
						</a>
						<a
							href="#security"
			...
			<section className="max-w-7xl mx-auto px-6 pt-20 pb-16 text-center relative z-10">
				<div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-xs font-mono text-primary mb-6 animate-pulse-subtle">
					...
					<span>
						OWASP ASVS 5.0 • ZERO TELEMETRY • LOCAL-FIRST SCIENTIFIC RESOLUTION
					</span>
				</div>
				...
				<h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight max-w-5xl mx-auto leading-[1.1]">
					...
					<span className="text-gradient-cyan">Auditable, Verified</span>{" "}
					Solutions.
				</h1>

				<p className="mt-6 text-sm sm:text-base text-muted-foreground max-w-3xl mx-auto leading-relaxed">
					ResolveOS replaces messy Slack threads and intuitive guesses with an
					enforced{" "}
					<strong className="text-foreground font-semibold">
						12-Stage Scientific Resolution Lifecycle
					</strong>
					. Featuring zero-telemetry local-first storage, automated PII
					sanitization, 3D relational telemetry graphs, and mathematical
					invariant blocks that prevent premature case closure.
				</p>
			...
			<section
				id="lifecycle"
				className="max-w-7xl mx-auto px-6 py-20 border-t border-border/60"
			>
				<div className="text-center max-w-3xl mx-auto mb-12">
					...
					<p className="mt-3 text-xs sm:text-sm text-muted-foreground">
						Click any stage below to inspect how ResolveOS turns chaotic guesses
				...
				{(() => {
					const current =
						stages.find((s) => s.num === activeStage) || stages[0];
					return (
						<div className="max-w-4xl mx-auto glass-panel-glow rounded-2xl p-8 transition-all">
							...
							<div className="mt-6 bg-background/90 rounded-xl border border-border/80 p-5 font-mono text-xs space-y-3">
								<div className="flex items-center justify-between text-muted-foreground pb-2 border-b border-border/40">
									<span>
										RESOLVEOS TELEMETRY RUNTIME // STAGE #{current.num}
									</span>
									<span className="text-primary font-bold">
										{current.preview.meter}
									</span>
								</div>
							...
							<div className="mt-6 flex items-center justify-between">
								<button
									onClick={() =>
										setActiveStage((prev) => (prev > 1 ? prev - 1 : 12))
									}
								...
								<button
									onClick={() =>
										setActiveStage((prev) => (prev < 12 ? prev + 1 : 1))
									}
									className="text-xs text-muted-foreground hover:text-foreground font-mono flex items-center gap-1"
								>
									Next Stage →
			...
			>
				<div className="text-center max-w-3xl mx-auto mb-12">
					...
					<p className="mt-3 text-xs sm:text-sm text-muted-foreground">
						Type or paste sensitive logs below. ResolveOS automatically strips
						credentials, API keys, tokens, and PII before any processing.
					</p>
				</div>
				...
				<div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div className="glass-panel rounded-2xl p-6 flex flex-col justify-between">
						...
						<div className="mt-3 text-[11px] text-muted-foreground font-mono">
							Detects: API keys (sk_*, ghp_*), JWT tokens, RFC emails, credit
							cards, and private IP blocks.
						</div>
					</div>
			...
			>
				<div className="text-center max-w-3xl mx-auto mb-12">
					...
					<p className="mt-3 text-xs sm:text-sm text-muted-foreground">
						Zero third-party tracking beacons. Zero telemetry. Strict
						server-side tenancy scoping.
					</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
					<div className="glass-panel rounded-2xl p-6 space-y-3">
						...
						<h3 className="text-base font-bold text-foreground">
							scrypt + Timing-Safe Equals
						</h3>
						<p className="text-xs text-muted-foreground leading-relaxed">
							Passwords hashed with Node-native scrypt (N=16384, r=8, p=1,
							64-byte key length). All auth and secret checks use{" "}
							<code>crypto.timingSafeEqual</code> to defeat side-channel timing
							attacks.
						</p>
					</div>
					...
					<div className="glass-panel rounded-2xl p-6 space-y-3">
						...
						<h3 className="text-base font-bold text-foreground">
							SSRFGuard Defense Guard
						</h3>
						<p className="text-xs text-muted-foreground leading-relaxed">
							Hardened server-side request verification blocks link-local,
							loopback, private IPv4/IPv6 blocks, and cloud instance metadata
							(169.254.169.254).
						</p>
					</div>
					...
					<div className="glass-panel rounded-2xl p-6 space-y-3">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							All transactions write to temporary scratch files (
							<code>.tmp.&lt;timestamp&gt;</code>) followed by atomic operating
							system file renames (<code>fs.renameSync</code>), preventing
							corruption upon crashes.
						</p>
					</div>
					...
					<div className="glass-panel rounded-2xl p-6 space-y-3">
						...
						<h3 className="text-base font-bold text-foreground">
							RFC 6238 TOTP Two-Factor Auth
						</h3>
						<p className="text-xs text-muted-foreground leading-relaxed">
							Hardware-compatible authenticator support with time-drift
							tolerance (±1 step), encrypted recovery codes, and remote
					...
					<div className="glass-panel rounded-2xl p-6 space-y-3">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							Fastify WebSockets verify JWT tokens during initial connection
							handshake. Unauthorized attempts are rejected with strict
					...
					<div className="glass-panel rounded-2xl p-6 space-y-3">
						...
						<h3 className="text-base font-bold text-foreground">
							Zero Telemetry & Right to Erasure
						</h3>
						<p className="text-xs text-muted-foreground leading-relaxed">
							Zero Google Analytics, tracking pixels, or third-party cookies.
							Full GDPR Art. 17 right-to-erasure and 1-click JSON/CSV workspace
							portability.
						</p>
					</div>
				</div>
			...
			>
				<div className="text-center max-w-3xl mx-auto mb-12">
					...
					<p className="mt-3 text-xs sm:text-sm text-muted-foreground">
						Zero external C++ native build dependencies. Runs seamlessly on
				...
				<div className="max-w-4xl mx-auto glass-panel rounded-2xl p-6 font-mono text-xs divide-y divide-border/60">
					<div className="py-3 flex items-center justify-between">
						...
						<span className="text-foreground font-semibold">
							TypeScript 5.7 • Node.js 20+ (ES Modules)
						</span>
					</div>
					<div className="py-3 flex items-center justify-between">
						...
						<span className="text-foreground font-semibold">
							Fastify 5.2 • Schema Validated • CORS & Helmet
						</span>
					</div>
					<div className="py-3 flex items-center justify-between">
						<span className="text-muted-foreground">
						...
						<span className="text-foreground font-semibold">
							React 18.3 • Vite 6.2 • Tailwind CSS 3.4
						</span>
					</div>
					<div className="py-3 flex items-center justify-between">
						<span className="text-muted-foreground">
						...
						<span className="text-foreground font-semibold">
							Zustand 5.0 • TanStack React Query 5.66
						</span>
					</div>
					<div className="py-3 flex items-center justify-between">
						<span className="text-muted-foreground">3D Relational Network</span>
						<span className="text-foreground font-semibold">
							Three.js 0.174 WebGL 2.0 Spatial Graph
						</span>
					</div>
					<div className="py-3 flex items-center justify-between">
						<span className="text-muted-foreground">
					...
					<div className="py-3 flex items-center justify-between">
						...
						<span className="text-emerald-400 font-semibold">
							Vitest 3.2 (33 / 33 Passing Suites)
						</span>
					</div>
				</div>
			</section>
			...
			>
				<div className="text-center max-w-3xl mx-auto mb-12">
					...
					<p className="mt-3 text-xs sm:text-sm text-muted-foreground">
						Engineered for organizations and teams that cannot afford ambiguous
						guesswork, lost diagnostic context, or unverified outage
						conclusions.
					</p>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
					...
					<div className="glass-panel rounded-3xl p-6 space-y-4 hover:border-primary/50 transition-all group">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							Eliminate chaotic Slack war-rooms during Sev-1 outages. Enforce
							real-time lead tracking, telemetry delta quantification, and the
							verification invariant before declaring recovery.
						</p>
					...
					<div className="glass-panel rounded-3xl p-6 space-y-4 hover:border-primary/50 transition-all group">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							Investigate intrusions without leaking credentials. Automated
							server-side PII/Secret redaction masks keys and JWTs, while
							immutable audit trails preserve digital chain-of-custody.
						</p>
						<div className="pt-2 border-t border-border/60 text-[11px] font-mono text-primary flex items-center gap-1">
					...
					<div className="glass-panel rounded-3xl p-6 space-y-4 hover:border-primary/50 transition-all group">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							Tackle complex, recurring architectural failures. Traverse
							recursive 5-Whys causality chains, construct Ishikawa diagrams,
							and record immutable Architectural Decision Records (ADRs).
						</p>
						<div className="pt-2 border-t border-border/60 text-[11px] font-mono text-primary flex items-center gap-1">
					...
					<div className="glass-panel rounded-3xl p-6 space-y-4 hover:border-primary/50 transition-all group">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							Diagnose distributed memory leaks, network partition deadlocks,
							and cross-service dependencies with the WebGL 2.0 3D spatial
							network and multi-criteria solution matrix.
						</p>
						<div className="pt-2 border-t border-border/60 text-[11px] font-mono text-primary flex items-center gap-1">
					...
					<div className="glass-panel rounded-3xl p-6 space-y-4 hover:border-primary/50 transition-all group">
						...
						<div>
							...
							<h3 className="text-base font-bold text-foreground mt-0.5">
								Compliance, CISOs & DPOs
							</h3>
						</div>
						<p className="text-xs text-muted-foreground leading-relaxed">
							Satisfy GDPR, CCPA, and SEC cyber disclosure mandates. Defend your
							organization with zero-telemetry hosting, complete data
							portability, and verifiable proof of resolution.
						</p>
						<div className="pt-2 border-t border-border/60 text-[11px] font-mono text-primary flex items-center gap-1">
					...
					<div className="glass-panel rounded-3xl p-6 space-y-4 hover:border-primary/50 transition-all group">
						...
						<p className="text-xs text-muted-foreground leading-relaxed">
							Elevate post-mortem culture from blameless to verified. Ensure
							preventive monitoring controls and systemic action items are
							assigned, tracked on Kanban, and completed.
						</p>
						<div className="pt-2 border-t border-border/60 text-[11px] font-mono text-primary flex items-center gap-1">
				...
				<div className="glass-panel-glow rounded-3xl p-10 sm:p-14 relative overflow-hidden">
					<div className="relative z-10 space-y-6">
						...
						<p className="text-xs sm:text-sm text-muted-foreground max-w-2xl mx-auto">
							Test drive the full 12-stage problem resolution lifecycle right
							now. Zero setup required.
						</p>
			...
			<footer className="border-t border-border/60 py-10 px-6 text-xs text-muted-foreground">
				<div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
					<div className="flex items-center gap-2 font-mono">
						...
						<span>
							ResolveOS v1.0.0 — Engineered for Mission-Critical Reliability
						</span>
					</div>
