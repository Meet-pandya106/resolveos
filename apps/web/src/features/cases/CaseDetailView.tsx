...
export const CaseDetailView: React.FC = () => {
	...
	const loadCaseFull = async () => {
		...
		try {
			...
			const [
				...
				APIClient.get<Case>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}`,
				),
				APIClient.get<CaseEvidence[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/evidence`,
				),
				APIClient.get<CaseQuestion[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/questions`,
				),
				APIClient.get<Hypothesis[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/hypotheses`,
				),
				APIClient.get<RootCause[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/root-causes`,
				),
				APIClient.get<Solution[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/solutions`,
				),
				APIClient.get<Decision[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/decisions`,
				),
				APIClient.get<CaseAction[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/actions`,
				),
				APIClient.get<Verification[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/verifications`,
				),
				APIClient.get<Retrospective | null>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/retrospective`,
				),
				APIClient.get<CaseActivity[]>(
					`/workspaces/${currentWorkspaceId}/cases/${caseId}/activities`,
				),
			]);

			setCaseData(cData);
			setProblemForm(
				cData.problemStatement || {
					title: cData.title,
					statement: cData.description,
			...
			addToast({
				type: "error",
				message: err.message || "Failed to load case.",
			});
		} finally {
			setLoading(false);
		}
	...
	const handleStatusChange = async (nextStatus: CaseStatus) => {
		...
		try {
			...
			addToast({
				type: "success",
				message: `Case status changed to ${nextStatus}`,
			});
			loadCaseFull();
		} catch (err: any) {
	...
	const handleAddVerification = async (e: React.FormEvent) => {
		...
		try {
			...
			addToast({
				type: "success",
				message: "Verification criterion registered.",
			});
			loadCaseFull();
		} catch (err: any) {
			addToast({ type: "error", message: err.message });
		...
		<div className="max-w-7xl mx-auto space-y-6 pb-12">
			...
			<div className="p-6 rounded-xl bg-card border border-border space-y-4 shadow-sm">
					...
					<div className="space-y-1">
						<div className="flex items-center gap-2">
							...
							<span className="text-xs font-mono text-muted-foreground">
								CASE-{caseData.id.slice(0, 8).toUpperCase()} • v
								{caseData.version}
							</span>
						</div>
				...
				<div className="p-4 rounded-xl bg-muted/20 border border-border/70 space-y-2">
					<div className="flex items-center justify-between text-xs font-mono">
						...
						<span className="text-muted-foreground text-[11px]">
							Formulation Quality:{" "}
							<strong className="text-primary">{problemScore.score}%</strong>
						</span>
					</div>
						...
						{[
							{ num: 1, name: "Formulate", done: problemScore.score >= 50 },
							{
								...
								name: "Symptom",
									...
									problemForm.expectedBehavior && problemForm.observedBehavior
								),
							},
				...
				<div className="p-3 rounded-xl bg-primary/5 border border-primary/20 flex items-center justify-between text-xs font-mono">
					<div className="flex items-center gap-2 text-primary">
						...
						<span>
							Invariant Guard: State transition to RESOLVED is physically
							blocked until a PASSED verification test is logged.
						</span>
					</div>
				...
				<div className="flex items-center gap-1 overflow-x-auto border-t border-border pt-3">
					{[
						...
						},
						{
							...
							icon: ShieldCheck,
							count: verificationsList.length,
						},
						{
							id: "retro",
							label: "8. Retrospective",
							icon: RotateCcw,
							count: retrospective ? "1" : "0",
						},
						{
							id: "timeline",
							label: "Timeline",
						...
						return (
							...
							>
								...
								{tab.count !== undefined && (
									<span
										className={`text-[10px] px-1.5 py-0.2 rounded font-mono ${isActive ? "bg-primary-foreground/20 text-primary-foreground" : "bg-muted text-muted-foreground"}`}
									>
										{tab.count}
									</span>
								)}
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
					<div className="lg:col-span-2 space-y-4 p-6 rounded-xl bg-card border border-border">
						<h2 className="text-base font-bold">
							Structured Problem Statement Builder
						</h2>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									...
									<input
										...
										onChange={(e) =>
											setProblemForm({ ...problemForm, impact: e.target.value })
										}
					...
					<div className="space-y-4 p-6 rounded-xl bg-card border border-border flex flex-col justify-between">
							...
							<div className="flex items-baseline justify-between">
								...
								<span
									className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase ${
										problemScore.rating === "COMPREHENSIVE"
											? "bg-emerald-500/20 text-emerald-400"
											: problemScore.rating === "ACTIONABLE"
												? "bg-blue-500/20 text-blue-400"
												: problemScore.rating === "BASIC"
							...
							<div className="w-full h-2 rounded-full bg-muted overflow-hidden">
								<div
									className={`h-full transition-all duration-500 ${
										problemScore.score >= 80
											? "bg-emerald-500"
											: problemScore.score >= 50
												? "bg-blue-500"
												: "bg-amber-500"
									}`}
								...
								<div className="space-y-2 pt-3">
									...
									<ul className="space-y-1">
										{problemScore.missingFields.map((m: string) => (
											<li
												key={m}
												className="text-xs text-amber-400 font-mono flex items-center gap-1.5"
											>
						...
						<div className="p-3 rounded-lg bg-muted/20 border border-border/50 text-[11px] text-muted-foreground">
							A clear problem statement eliminates 80% of investigative
							confusion.
						</div>
					</div>
				</div>
			...
			{activeTab === "evidence" && (
				<div className="space-y-6">
					<form
						...
						className="p-5 rounded-xl bg-card border border-border flex gap-3"
					>
						<input
							type="text"
							required
				...
				<div className="space-y-6 p-6 rounded-xl bg-card border border-border">
					...
					<div className="space-y-3">
						{rootCausesList.map((rc, idx) => (
							<div
								key={rc.id}
								className="p-3 rounded-lg border border-border/80 bg-muted/20 flex items-center gap-3"
							>
							...
							>
								...
								<div className="grid grid-cols-4 gap-2 text-center text-[10px] font-mono pt-2 border-t border-border/50">
									<div className="p-1 rounded bg-muted/30">
										Cost: {sol.costScore}/5
									</div>
									<div className="p-1 rounded bg-muted/30">
										Effort: {sol.effortScore}/5
									</div>
									<div className="p-1 rounded bg-muted/30">
										Risk: {sol.riskScore}/5
									</div>
									<div className="p-1 rounded bg-muted/30">
										Impact: {sol.impactScore}/5
									</div>
								</div>
							</div>
						))}
			...
			{activeTab === "verification" && (
				<div className="space-y-6 p-6 rounded-xl bg-card border border-border">
					...
					<p className="text-xs text-muted-foreground">
						A case cannot be marked resolved without at least one PASSED
				...
				<div className="p-6 rounded-xl bg-card border border-border space-y-4">
					...
					{retrospective ? (
						<div className="space-y-4 text-xs leading-relaxed">
							<div>
								<span className="font-semibold text-muted-foreground font-mono uppercase">
									What Happened:
								</span>
