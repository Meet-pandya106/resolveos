...
import { Workspace, WorkspaceMember, UserRole } from '@resolveos/shared';
import {
  Settings,
  Building,
  Users,
...
export const SettingsView: React.FC = () => {
  const { user, workspaces, activeWorkspaceId, setActiveWorkspace } = useAuthStore();
  const { addToast } = useUIStore();

  const [activeTab, setActiveTab] = useState<'general' | 'governance' | 'members' | 'privacy' | 'notifications' | 'danger'>('general');
  ...
  const [retentionDays, setRetentionDays] = useState(activeWorkspace?.retentionDays || 365);
  const [aiEnabled, setAiEnabled] = useState(activeWorkspace?.aiProcessingEnabled || false);
  ...
  const handleSaveGeneral = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({ type: 'success', message: 'Workspace settings updated successfully.' });
    } catch (err: any) {
      addToast({ type: 'error', message: err.message || 'Failed to update settings.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleInviteMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({ type: 'success', message: `Added ${newMemberEmail} as ${newMemberRole}.` });
      setNewMemberEmail('');
      // Reload members
      const updated = await APIClient.get<any[]>(`/workspaces/${activeWorkspaceId}/members`);
      setMembers(updated || []);
    } catch (err: any) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-16">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-muted text-muted-foreground font-semibold">
                WORKSPACE ID: {activeWorkspace?.id ? activeWorkspace.id.slice(0, 12) : 'DEFAULT'}
              </span>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            Manage tenancy parameters, 12-stage problem governance, member roles, and compliance policies.
          </p>
        </div>
      </div>
      ...
      {activeTab === 'general' && (
        <form onSubmit={handleSaveGeneral} className="glass-panel rounded-3xl p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold mb-1 text-foreground">Workspace Name *</label>
              <input
                type="text"
                required
                value={name}
            ...
            <div>
              <label className="block text-xs font-semibold mb-1 text-foreground">Workspace URL Slug *</label>
              <input
                type="text"
                required
                value={slug}
          ...
          <div>
            <label className="block text-xs font-semibold mb-1 text-foreground">Description / Purpose</label>
            <textarea
              rows={3}
              value={description}
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground font-mono mt-1.5">
              Closed cases older than this threshold will be flagged for automated archival pruning.
            </p>
          </div>
      ...
      {activeTab === 'governance' && (
        <div className="glass-panel rounded-3xl p-6 sm:p-8 space-y-6">
          <div className="border-b border-border/60 pb-4">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Enforce empirical rigor and prevent premature incident resolution across your engineering teams.
            </p>
          </div>
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Physically blocks any case from transitioning to <code>RESOLVED</code> status unless at least one empirical verification test with a status of <code>PASSED</code> is recorded.
                </p>
              </div>
              <input
                type="checkbox"
                ...
                onChange={e => {
                  ...
                  addToast({ type: 'info', message: 'Verification invariant updated.' });
                }}
            ...
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Require teams to follow formulation, symptom delta quantification, 5-Whys root cause, and multi-criteria weighted scoring before marking fixes.
                </p>
              </div>
              <input
                type="checkbox"
                ...
                onChange={e => {
                  ...
                  addToast({ type: 'info', message: '12-Stage workflow mode updated.' });
                }}
                className="w-4 h-4 rounded text-primary cursor-pointer"
              />
            </div>
            ...
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Standard starting severity assigned to newly created investigation tickets.
                </p>
              </div>
              <select
                ...
                onChange={e => {
                  ...
                  addToast({ type: 'success', message: `Default severity set to ${e.target.value}.` });
                }}
      ...
      {activeTab === 'members' && (
        <div className="glass-panel rounded-3xl p-6 sm:p-8 space-y-6">
          ...
          {/* Add Member Form */}
          <form onSubmit={handleInviteMember} className="p-4 rounded-2xl bg-muted/20 border border-border/70 space-y-3">
            ...
            <p className="text-[10px] text-muted-foreground font-mono">
              Note: The user must already have a registered account in ResolveOS.
            </p>
          </form>
                  ...
                  <div key={m.id} className="p-3.5 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-primary/10 border border-primary/20 text-primary flex items-center justify-center font-bold text-xs font-mono">
                        {m.user?.name ? m.user.name.slice(0, 2).toUpperCase() : 'RO'}
                      </div>
                      <div>
                        <div className="text-xs font-bold text-foreground">{m.user?.name || 'Engineer'}</div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold ${
                        m.role === 'OWNER' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30' :
                        m.role === 'ADMIN' ? 'bg-primary/20 text-primary border border-primary/30' :
                        m.role === 'MEMBER' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Allow optional synthetic assistance for hypothesis brainstorming. When enabled, all prompts are pre-sanitized through the server-side <code>DataRedactor</code> engine before egress.
                </p>
              </div>
              <input
                type="checkbox"
                ...
                onChange={async e => {
                  ...
                  if (activeWorkspaceId) {
                    await APIClient.patch(`/workspaces/${activeWorkspaceId}`, { aiProcessingEnabled: next });
                    addToast({ type: 'success', message: `AI Processing gate ${next ? 'enabled' : 'disabled'}.` });
                  }
                }}
                className="w-4 h-4 rounded text-primary cursor-pointer"
              />
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                <div className="text-xs font-bold text-foreground">Critical Outage Audio Radar</div>
                <p className="text-[11px] text-muted-foreground">
                  Play emergency acoustic tone when a Sev-1 incident is declared.
                </p>
              </div>
              <input
                type="checkbox"
                ...
                onChange={e => {
                  ...
                  addToast({ type: 'info', message: 'Audio alert preferences saved.' });
                }}
                className="w-4 h-4 rounded text-primary cursor-pointer"
              />
            </div>
            ...
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                <div className="text-xs font-bold text-foreground">Persistent Incident Banners</div>
                <p className="text-[11px] text-muted-foreground">
                  Show top-level pulsing radar banner across all pages when incident mode is active.
                </p>
              </div>
              <input
                type="checkbox"
                ...
                onChange={e => {
                  ...
                  addToast({ type: 'info', message: 'Banner alert preferences saved.' });
                }}
                className="w-4 h-4 rounded text-primary cursor-pointer"
              />
            </div>
      ...
      {activeTab === 'danger' && (
        <div className="glass-panel rounded-3xl p-6 sm:p-8 space-y-6 border-rose-500/30">
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-card border border-border/70 flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                <div className="text-xs font-bold text-foreground">Export All Workspace Records</div>
                <p className="text-[11px] text-muted-foreground">
                  Download all cases, evidence artifacts, 5-Whys trees, and verification logs.
                </p>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-between gap-4">
              <div className="space-y-0.5">
                <div className="text-xs font-bold text-rose-400">Decommission Workspace</div>
                <p className="text-[11px] text-rose-300/80">
                  Permanently erase all problem cases and remove member access. This action cannot be reversed.
                </p>
              </div>
              <button
                type="button"
                onClick={() => addToast({ type: 'warning', message: 'Workspace decommissioning requires root administrative confirmation.' })}
