...
import { Network, Activity, Layers, Sparkles, Eye, ShieldCheck } from 'lucide-react';
...
export const EvidenceGraphView: React.FC = () => {
  ...
  const nodeData = [
    { name: 'Problem Formulation', color: 0xef4444, radius: 6.5, type: 'Definition', desc: 'Structured statement & completeness score' },
    { name: 'Corroborating Evidence', color: 0x3b82f6, radius: 9.0, type: 'Telemetry', desc: 'APM logs, heap dumps, confidence ratings' },
    { name: 'Competing Hypotheses', color: 0xeab308, radius: 11.5, type: 'Theory', desc: 'Rival root cause theories in evaluation' },
    { name: '5-Whys Root Cause Chain', color: 0xa855f7, radius: 7.8, type: 'Causality', desc: 'Recursive causality depth analysis' },
    { name: 'Solution Matrix', color: 0x10b981, radius: 10.2, type: 'Mitigation', desc: 'Multi-criteria weighted scoring' },
    { name: 'Empirical Verification', color: 0x06b6d4, radius: 12.8, type: 'Verification', desc: 'Pre vs post remediation telemetry delta' }
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Spatial visualization mapping active cases, evidence nodes, hypotheses, and verification telemetry in 3D.
          </p>
        </div>
      ...
      <div className="p-4 sm:p-6 rounded-3xl glass-panel-glow overflow-hidden relative shadow-2xl">
        ...
        <div className="absolute top-6 left-6 z-10 p-3.5 rounded-2xl bg-card/85 backdrop-blur-xl border border-border/80 text-xs font-mono space-y-1 shadow-lg max-w-xs">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Dynamic orbital nodes correlate failure symptoms, competing theories, and verified fixes.
          </p>
        </div>

        {/* 3D Canvas Mount */}
        <div ref={mountRef} className="w-full h-[550px] rounded-2xl overflow-hidden cursor-grab active:cursor-grabbing" />
