...
export const DashboardView: React.FC = () => {
  ...
  const [filterMode, setFilterMode] = useState<'ALL' | 'INCIDENT' | 'VERIFYING' | 'RESOLVED'>('ALL');
  ...
  const stageStats = [
    { label: 'Formulation', count: cases.filter(c => c.status === 'DRAFT').length, color: 'bg-blue-500' },
    { label: 'Investigation', count: cases.filter(c => c.status === 'INVESTIGATING').length, color: 'bg-amber-500' },
    { label: 'Verification', count: cases.filter(c => c.status === 'VERIFYING').length, color: 'bg-purple-500' },
    { label: 'Resolved', count: cases.filter(c => c.status === 'RESOLVED').length, color: 'bg-emerald-500' }
  ];
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Enforced 12-stage scientific problem lifecycle monitoring and incident governance.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <h2 className="text-sm font-bold text-foreground mt-0.5">
                {incidentCases.length} Critical Outage{incidentCases.length > 1 ? 's' : ''} in Rapid Containment Mode
              </h2>
              <p className="text-[11px] text-muted-foreground">
                Live collaboration, emergency root-cause verification, and telemetry active.
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl glass-panel border border-border/80 relative overflow-hidden group hover:border-primary/50 transition-all">
          ...
          <div className="text-3xl font-black mt-2 font-mono">{openCases.length}</div>
          <div className="mt-2 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            <span className="text-primary font-semibold">12-Stage</span> pipeline active
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-border/80 relative overflow-hidden group hover:border-rose-500/50 transition-all">
          ...
          <div className="text-3xl font-black mt-2 text-rose-400 font-mono">{criticalCases.length}</div>
          <div className="mt-2 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            <span className="text-rose-400 font-semibold">{incidentCases.length}</span> under incident mode
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-border/80 relative overflow-hidden group hover:border-amber-500/50 transition-all">
          ...
          <div className="text-3xl font-black mt-2 text-amber-400 font-mono">{verifyingCases.length}</div>
          <div className="mt-2 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            <span className="text-amber-400 font-semibold">Invariant Guard:</span> Blocked till pass
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl glass-panel border border-border/80 relative overflow-hidden group hover:border-emerald-500/50 transition-all">
          ...
          <div className="text-3xl font-black mt-2 text-emerald-400 font-mono">{resolvedCases.length}</div>
          <div className="mt-2 flex items-center gap-1.5 text-[11px] text-muted-foreground">
            <TrendingUp className="w-3 h-3 text-emerald-400" />
            <span className="text-emerald-400 font-semibold">100% verified</span> zero regression
          </div>
        </div>
      </div>
      ...
      <div className="p-5 rounded-2xl glass-panel border border-border/80 space-y-3">
        ...
        <div className="w-full h-3 rounded-full bg-muted/40 overflow-hidden flex gap-0.5 p-0.5">
          {stageStats.map((st, i) => {
            const pct = cases.length > 0 ? Math.max((st.count / cases.length) * 100, 3) : 25;
            return (
              <div
                key={i}
                style={{ width: `${pct}%` }}
      ...
      <div className="p-5 sm:p-6 rounded-2xl glass-panel border border-border/80 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/60 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-foreground">Active Case Stream</h3>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {(['ALL', 'INCIDENT', 'VERIFYING', 'RESOLVED'] as const).map(mode => (
              <button
                key={mode}
              ...
              >
                {mode === 'ALL' ? 'All Cases' : mode === 'INCIDENT' ? '🚨 Incidents' : mode === 'VERIFYING' ? '⏳ Verifying' : '✓ Resolved'}
              </button>
            ))}
          </div>
        </div>
            ...
            {displayCases.map(c => (
              ...
              >
                ...
                <div className="pt-2 border-t border-border/40 flex items-center justify-between text-[11px] font-mono text-muted-foreground">
                  <div className="flex items-center gap-2">
                    <span className="text-foreground font-semibold">12-Stage Engine</span>
                    <span>•</span>
