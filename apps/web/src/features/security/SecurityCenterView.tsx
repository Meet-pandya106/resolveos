...
import { UserSession, AuditEvent, APIKey } from '@resolveos/shared';
import {
  ShieldCheck,
  Smartphone,
  Laptop,
...
export const SecurityCenterView: React.FC = () => {
  ...
  const [sessions, setSessions] = useState<(UserSession & { isCurrent?: boolean })[]>([]);
  ...
  const loadSecurityData = async () => {
    try {
      ...
      if (activeWorkspaceId) {
        const keysData = await APIClient.get<APIKey[]>(`/workspaces/${activeWorkspaceId}/api-keys`).catch(() => []);
  ...
  const handleCreateAPIKey = async (e: React.FormEvent) => {
    ...
    try {
      const res = await APIClient.post(`/workspaces/${activeWorkspaceId}/api-keys`, {
        name: newKeyName,
        scopes: ['read', 'write']
      });
      setCreatedSecret(res.secretKey);
      setNewKeyName('');
      addToast({ type: 'success', message: 'API key generated. Store it safely.' });
      loadSecurityData();
    } catch (err: any) {
      addToast({ type: 'error', message: err.message });
    }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Security & Access Governance</h1>
        <p className="text-xs text-muted-foreground mt-1">
          Active session rotation, cryptographic key management, and tamper-resistant audit logs.
        </p>
      </div>
      ...
      <div className="p-6 rounded-xl bg-card border border-border space-y-4">
        ...
        <div className="space-y-3">
          {sessions.map(s => (
            <div key={s.id} className="p-3.5 rounded-lg bg-muted/20 border border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] font-mono text-muted-foreground">
                    IP: {s.ipAddress} • Last active: {new Date(s.lastActiveAt).toLocaleTimeString()}
                  </p>
                </div>
              </div>
      ...
      <div className="p-6 rounded-xl bg-card border border-border space-y-4">
        ...
        {createdSecret && (
          <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/30 space-y-2">
            <span className="text-xs font-bold text-emerald-400">Save Your API Key Secret Now:</span>
            <div className="flex items-center gap-2">
              <input
                type="text"
                readOnly
        ...
        <div className="space-y-2 pt-2">
          {apiKeysList.map(k => (
            <div key={k.id} className="p-3 rounded-lg bg-muted/20 border border-border flex items-center justify-between text-xs font-mono">
      ...
      <div className="p-6 rounded-xl bg-card border border-border space-y-4">
        ...
        <div className="space-y-2">
          {auditLogs.map(log => (
            <div key={log.id} className="p-3 rounded-lg bg-muted/20 border border-border/60 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2">
