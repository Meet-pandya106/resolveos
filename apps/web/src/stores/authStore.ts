...
interface AuthState {
  ...
  isLoading: boolean;
  login: (user: User, token: string, workspaces?: Workspace[], defaultWorkspaceId?: string) => void;
  logout: () => void;
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  fetchProfile: async () => {
    ...
    try {
      ...
      const activeWs = (data.workspaces && data.workspaces.some((w: any) => w.id === savedActiveWs))
        ? savedActiveWs
