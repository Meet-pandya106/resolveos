...
export const ProblemStatementSchema = z.object({
  ...
  statement: z.string().min(10, 'Problem statement must be at least 10 characters').max(5000),
  ...
  frequency: z.enum(['CONSTANT', 'INTERMITTENT', 'ONE_OFF', 'UNKNOWN']).default('UNKNOWN'),
...
export const CreateEvidenceInputSchema = z.object({
  ...
  fileSize: z.number().int().max(50 * 1024 * 1024).optional().nullable(), // 50MB max
...
export const CreateRelationshipInputSchema = z.object({
  sourceType: z.enum(['evidence', 'hypothesis', 'root_cause', 'solution', 'problem']),
  sourceId: z.string().uuid(),
  targetType: z.enum(['evidence', 'hypothesis', 'root_cause', 'solution', 'action']),
...
export const CreateSolutionInputSchema = z.object({
  ...
  reversibility: z.enum(['HIGH', 'MEDIUM', 'LOW', 'IRREVERSIBLE']).default('MEDIUM'),
...
export const ActionStatusSchema = z.enum(['TODO', 'IN_PROGRESS', 'BLOCKED', 'DONE', 'CANCELLED']);
...
export const VerificationStatusSchema = z.enum(['PENDING', 'PASSED', 'FAILED', 'INCONCLUSIVE']);
