...
export class TOTPService {
	 ...
	 */
	static getOtpAuthUri(
		account: string,
		secretBase32: string,
		issuer: string = "ResolveOS",
	): string {
...
export class DataRedactor {
	private static readonly PATTERNS = [
		...
		// Matches sk-live-..., sk_live_..., api_key_..., ghp_..., xoxb-..., etc.
		{
			type: "API_KEY",
			regex:
				/(?:sk|pk|ak|api|key|token|secret)[-_](?:live|test|prod|dev)?[a-zA-Z0-9_-]{12,64}/gi,
		...
		},
		{
			...
			regex: /https?:\/\/[a-zA-Z0-9_]+:[^@\s]+@[a-zA-Z0-9.-]+/gi,
	...
	static isSafeUrl(rawUrl: string): { safe: boolean; reason?: string } {
		try {
			...
			if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
				return {
					safe: false,
					reason: `Protocol "${parsed.protocol}" is forbidden. Only HTTP/HTTPS permitted.`,
				};
			}
			...
			if (
				...
				this.BLOCKED_HOSTS.includes(parsed.hostname.toLowerCase())
			) {
				return {
					safe: false,
					reason: `Access to host "${hostname}" is prohibited.`,
				};
			}
			...
			if (
				...
				return {
					safe: false,
					reason: "Private/Loopback IPv6 address blocked.",
				};
			}
			...
			if (this.isPrivateIPv4(hostname)) {
				return {
					safe: false,
					reason: "Private/Loopback IPv4 network address blocked.",
				};
			}
			...
			for (const record of addresses) {
				if (record.family === 4) {
					if (this.isPrivateIPv4(record.address)) {
						return {
							safe: false,
							reason: `Host resolves to private IPv4 address (${record.address}). SSRF blocked.`,
						};
					}
					...
					if (
						...
						return {
							safe: false,
							reason: `Host resolves to private/loopback IPv6 address (${ip}). SSRF blocked.`,
						};
					}
				}
			}
